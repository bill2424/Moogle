...
module DictSet(C : COMPARABLE) : (SET with type elt = C.t) =
  ...
  let test_union () =
    ...
    List.iter (fun k -> assert(member union_elts k)) elts1;
    List.iter (fun k -> assert(member union_elts k)) elts2;
    ()
  ...
  let test_intersect () =
    ...
    List.iter (fun k -> assert(member s1 k && member s2 k = member inter_elts k)) (elts1 @ elts2);
    ()
  ...
  let test_choose () =
    ...
    let key2 = C.gen_random() in
    let dict2 = singleton key in
    let dict2 = insert key2 dict2 in
    assert(let x = choose dict2 in x = Some(key,dict2) || x = Some(key2,dict2));
    ()
      
  let test_fold () =
    let elts = generate_random_list 100 in
    let dict = insert_list empty elts in
    fold (fun elt x -> assert (member dict elt)) () dict;
    ()
