...
struct
  ...
  exception NoOutgoingEdges

  let rank (g: G.graph) =
    let map_of_node = NS.zero_node_score_map (G.nodes g) in
    
    (* helper function that picks random elements from ns *)
    let random_element ns =
      let len = (List.length ns) in
      if len = 0 then None
      else Some (List.nth ns (Random.int len))
    in

    (* helper function that matches the option node with some node or none,
       which raises an exception *)
    let match_option node =
      match node with
      | Some node -> node
      | None -> raise NoOutgoingEdges
    in

    (* helper function that picks random neighbor from neighbor_lst *)
    let random_neighbor neighbor_lst =
      match neighbor_lst with
      | None -> None
      | Some lst -> (random_element lst)
    in

    (* helper function that walks the Sisyphus *)
    let rec walk_through_graph g num_steps node map_of_node =
      if (G.is_empty g) then map_of_node
      else if num_steps = 0 then map_of_node
      else
      match random_neighbor (G.neighbors g node) with
      | None -> let new_n = (match_option (G.get_random_node g)) in
		let new_map = NS.add_score map_of_node new_n 1.0 in
		walk_through_graph g (num_steps-1) new_n new_map
      | Some n ->
         let chance =
           (match P.do_random_jumps with
            ...
            | Some prob -> prob) in
         let new_n =
           if (chance > (Random.float 1.0)) then
             match_option (G.get_random_node g)
           else n in	 
         let new_map = NS.add_score map_of_node new_n 1.0 in
         walk_through_graph g (num_steps - 1) new_n new_map
    in
    NS.normalize (walk_through_graph g P.num_steps 
      (match_option (G.get_random_node g)) map_of_node)
end
