...
struct
  ...
  let rec fold (f: key -> value -> 'a -> 'a) (u: 'a) (d: dict) : 'a =
    ...
    | Three (left,(k1,v1),middle,(k2,v2),right) -> 
        f k1 v1 (f k2 v2 (fold f (fold f (fold f u middle) left) right))
  ...
  let string_of_value = D.string_of_value
  let string_of_dict (d: dict) : string = 
    let make_string (k: key) (v: value) (u: string) : string =
      "Key: " ^ (string_of_key k) ^ "Value: " ^ (string_of_value v) ^ "\n" ^ u
    in
    fold make_string "" d
  ...
  let insert_upward_two (w: pair) (w_left: dict) (w_right: dict) 
    ...
    let (keyw, _) = w in
    let (keyx, _) = x in
    match (D.compare keyw keyx) with
  ...
  let insert_upward_three (w: pair) (w_left: dict) (w_right: dict)
    ...
    | Greater ->
       (match (D.compare keyw keyy) with
	| Eq -> Up(Two(other_left, x, w_left), w, Two(w_right, y, other_right))
  ...
  let rec insert_downward (d: dict) (k: key) (v: value) : kicked =
    match d with
      ...
      | Two(left,n,right) -> (match n with
          (kn, vn) -> insert_downward_two (k,v) (kn,vn) left right )
      | Three(left,n1,middle,n2,right) -> (match n1 with
          (kn1, vn1) -> (match n2 with 
            (kn2, vn2) -> 
  ...
  and insert_downward_two ((k,v): pair) ((k1,v1): pair) 
    ...
    match (D.compare k k1) with
    | Eq -> (match (insert_downward left k v) with
               Up(left2,node,right2) -> 
               insert_upward_two node left2 right2 (k1,v1) right
    ...
    | Greater -> (match (insert_downward right k v) with
        Up(left2,node,right2) -> 
          insert_upward_two node left2 right2 (k1,v1) left
  ...
  and insert_downward_three ((k,v): pair) ((k1,v1): pair) ((k2,v2): pair) 
    ...
    | Eq ->
       (match (insert_downward left k v) with
        | Up(left2,node,right2) -> 
          insert_upward_three node left2 right2 (k1,v1) (k2,v2) middle right
	...
	| Eq ->
	   (match (insert_downward middle k v) with 
	    | Up(left2,node,right2) -> 
              insert_upward_three node left2 right2 (k1,v1) (k2,v2) left right
	...
	| Greater ->
	   (match (insert_downward right k v) with
	    | Up(left2,node,right2) -> 
               insert_upward_three node left2 right2 (k1, v1) (k2, v2) left middle
  ...
  let insert (d: dict) (k: key) (v: value) : dict =
    if (member d k) then let d = (remove d k) in
      match insert_downward d k v with
        | Up(l,(k1,v1),r) -> Two(l,(k1,v1),r)
        | Done x -> x
    else match insert_downward d k v with
        | Up(l,(k1,v1),r) -> Two(l,(k1,v1),r)
        | Done x -> x
  ...
  let choose (d: dict) : (key * value * dict) option =
    ...
    | Three(left,(k1,v1),middle,(k2,v2),right) -> Some(k1,v1, (remove d k1))
   ...
   * ANSWER: 
   *    We recursively go down the tree. If a node contains
        just leaves, then it is balanced. If it contains a leaf and a node, then
        it is imabalanced. If it contains only nodes, then we continue to check
        the nodes' subtrees if they are balanced.
   *)
    ...
    match d with
      ...
      | Two(left,(k,v),right) -> 
          ( match (left, right) with
            ...
            | _, Leaf -> false
            | Leaf, _ -> false
            | _, _ -> (balanced left) && (balanced right) )
      | Three(left,(k1,v1),middle,(k2,v2),right) -> 
          ( match (left, middle, right) with
            ...
            | _, _, Leaf -> false
            | _, _, _ -> 
            (balanced left) && (balanced middle) && (balanced right) )
  ...
  let test_insert () =
    ...
    let dict = insert empty k1 v1 in
    let dict = insert dict k1 v1 in
    assert(balanced dict);
    assert(member dict k1);
    let dict = (remove dict k1) in
    assert(dict = empty);
    ()

  let test_lookup () =
    let pairs = generate_pair_list 25 in
    let dict = insert_list empty pairs in
    List.iter (fun (k, v) -> assert(match (lookup dict k) with
        Some v2 -> (v2 = v)
      | None -> false)) pairs ;
    ()
  ...
  let test_member () =
    ...
    assert(member dict k1);
    let dict = remove dict k1 in
    assert((member dict k1) = false);
    ()

  let test_choose () =
    let (k1,v1) = D.gen_pair () in
    let (k2,v2) = D.gen_pair () in
    let ks = [(k1,v1);(k2,v2)] in
    let dict = insert empty k1 v1 in
    let dict = insert dict k2 v2 in
    assert(balanced dict) ;
    assert(match (choose dict) with
        Some (k3, v3, d) -> 
          assert(balanced d) ;
          if (k3 = k1 && v3 = v1 && member d k1 = false) then
            let ks = List.filter (fun (k,_) -> k != k1) ks in
            (match choose d with
              Some (k3, v3, d) ->
                assert(balanced d) ;
                if (k3 = k2 && v3 = v2 && member d k2 = false) then
                  let ks = List.filter (fun (k,_) -> k != k2) ks in 
                  assert(List.length ks = 0) ;
                  true 
                else false
            | None -> false )
          else if (k3 = k2 && v3 = v2 && member d k2 = false) then
            let ks = List.filter (fun (k,_) -> k != k2) ks in
            (match choose d with
              Some (k3, v3, d) ->
                assert(balanced d) ;
